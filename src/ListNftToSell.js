...
const ListNftToSell = () => {
  ...
  useEffect(() => {
    // Hàm gọi API để lấy danh sách NFT
    const fetchNfts = async () => {
      try {
        const url = 'https://api.gameshift.dev/nx/items?assetStatus=Committed';  // Bỏ 'forSale=true'
        const options = {
          method: 'GET',
          headers: {
  ...
  // Hàm xử lý khi người dùng chọn bán một NFT
  const handleSelectNft = (nftId) => {
    navigate(`/sell-nft/${nftId}`);
  };
  ...
  return (
    ...
    <div style={styles.container}>
      <h1 style={styles.title}>NFTs Available for Sale</h1>
